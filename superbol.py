...
for i in use1:
    ...
    x = 1
    for j in filts1:
        lc[j] = np.array(zip(d[:,0][~np.isnan(d[:,x])],d[:,x][~np.isnan(d[:,x])],d[:,x+1][~np.isnan(d[:,x])]))
        x+=2
...
if useInt!='y':
    ...
    int_file = open(outdir+'/interpolated_lcs_'+sn+'_'+filters+'.txt','w')
    ...
    np.savetxt(int_file,int_out,fmt='%.2f',delimiter='\t')
...
#    co = np.abs(covar[0,1])
    ...
    if len(wlref[wlref<3000])>2:
        try:
            ...
            out2.write('%.2f\t%.2e\t%.2e\t%.2e\t%.2e\t%.2e\t%.2e\t%.2e\t%.2e\n' %(ph,T1,T1_err,abs(R1),R1_err,Topt,Topt_err,abs(Ropt),Ropt_err))
...
logout = zip(phase,np.log10(L1arr),0.434*L1err_arr/L1arr)
logoutBB = zip(phase,np.log10(L2arr),0.434*L2err_arr/L2arr)
